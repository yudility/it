...
public class DistPreprocessSmall{
    ...
    static class Distance{
        ...
        int prevNode; // (add) for saving previously visited node
    ...
    static class BidirectionalDijkstra{
        ...
        public ArrayList<Integer> computeShortestPath(Vertex[] graph, int source, int target, int[] nodeOrdering) {
            // Run bidirectional Dijkstra's algorithm to find shortest path
            computeDist(graph, source, target, 0, nodeOrdering);

            // Reconstruct shortest path by tracing back from target to source
            ArrayList<Integer> shortestPath = new ArrayList<>();
            int currentVertex = target;
            while (currentVertex != source) {
                shortestPath.add(currentVertex);
                currentVertex = graph[currentVertex].distance.prevNode;
            }
    ...
    public static void main(String args[]) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            y = in.nextInt()-1;
            c = in.nextLong();
            //vertex하나당 입력 3개, x: 출발점 , y: 도착점 , c:cost
        ...
        int t = in.nextInt(); //이게뭐지 양방향 다익스트라 가운데 점인가?아니네 최종적으로 구하고자하는 도착점임
        int u = 0;
        int v = 0;
        for (int i = 0; i < t; i++) {
            ...
            u = in.nextInt()-1;
            v = in.nextInt()-1;
            long distance = bd.computeDist(graph,u,v,i,nodeOrdering);

            System.out.println("Shortest distance from " + (u+1) + " to " + (v+1) + " is: " + distance);

            // 방문한 노드 출력
            //System.out.println("Visited nodes for query " + (i+1) + ": " + Arrays.toString(new long[]{graph[u].distance.queryDist}));


        }
        // Print the updated graph after preprocessing
        System.out.println("Updated Graph:");
        for (int i = 0; i < graph.length; i++) {
            Vertex vertex2 = graph[i];
            System.out.print("Vertex " + vertex2.vertexNum + ": ");
            System.out.print("In-Edges: ");
            for (int j = 0; j < vertex2.inEdges.size(); j++) {
                int inVertex = vertex2.inEdges.get(j) +1;
                long inCost = vertex2.inECost.get(j);
                System.out.print("(" + inVertex + ", " + inCost + ") ");
            }
            System.out.print("Out-Edges: ");
            for (int j = 0; j < vertex2.outEdges.size(); j++) {
                int outVertex = vertex2.outEdges.get(j)+1;
                long outCost = vertex2.outECost.get(j);
                System.out.print("(" + outVertex + ", " + outCost + ") ");
            }
            System.out.println();
        }
        ArrayList<Integer> shortestPath = bd.computeShortestPath(graph, u, v, nodeOrdering);
        System.out.println("Shortest Path:");

        for (int node : shortestPath) {
            System.out.print((node) + " -> ");
        }
        System.out.println(v+1); // 마지막 노드는 별도로 출력하여 경로의 끝을 나타냄
